...
class RehberActivity81 : AppCompatActivity() {
    ...
    private val passive = Color.rgb(174, 184, 197)
    ...
    private val roleLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) {
        if (activeScreen == "settings") showSettings() else if (activeScreen == "home") showHome()
    }
    ...
    private fun buildShell() {
        ...
        header.addView(headerBack, LinearLayout.LayoutParams(dp(38), dp(48)))
    ...
    private fun roundHeaderButton(iconRes: Int, color: Int, action: () -> Unit): View {
        return FrameLayout(this).apply {
            background = rounded(Color.rgb(244, 247, 251), dp(13).toFloat())
            setOnClickListener { action() }
            addView(ImageView(this@RehberActivity81).apply {
                setImageResource(iconRes)
                imageTintList = ColorStateList.valueOf(color)
                setPadding(dp(10), dp(10), dp(10), dp(10))
            }, FrameLayout.LayoutParams(dp(42), dp(42), Gravity.CENTER))
            layoutParams = LinearLayout.LayoutParams(dp(46), dp(46)).apply { marginStart = dp(6) }
        }
    }
    ...
    private fun showHome() {
        ...
        tileRow(
            ...
            tile(R.drawable.ic_rehber_settings, "Ayarlar", "", Color.rgb(116, 139, 168)) { showSettings() }
        )
        ...
        val slogan = LinearLayout(this).apply {
            ...
            background = rounded(Color.rgb(236, 245, 255), dp(17).toFloat())
        }
    ...
    private fun statusStrip(): View {
        ...
        return TextView(this).apply {
            text = "Arayan kimliği: " + caller + "  ·  Telefon kaydı: " + cache.count() + "\nSon eşitleme: " + last
            textSize = 10.3f
            setLineSpacing(dp(2).toFloat(), 1f)
            setTextColor(muted)
            setPadding(dp(11), dp(8), dp(11), dp(8))
            background = rounded(Color.WHITE, dp(14).toFloat(), soft)
        }.also { it.layoutParams = marginLp(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0, 0, 0, dp(9)) }
    }

    private fun tile(iconRes: Int, label: String, countLabel: String, color: Int, action: () -> Unit): View {
        return LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            gravity = Gravity.CENTER
            setPadding(dp(8), dp(10), dp(8), dp(9))
            ...
            addView(TextView(this@RehberActivity81).apply {
                text = label
                textSize = 15.5f
                gravity = Gravity.CENTER
                setTypeface(typeface, Typeface.BOLD)
            ...
            if (countLabel.isNotBlank()) addView(TextView(this@RehberActivity81).apply {
                text = countLabel
                textSize = 9.8f
                gravity = Gravity.CENTER
    ...
    private fun tileRow(parent: LinearLayout, left: View, right: View) {
        ...
        parent.addView(row, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(138)))
    }
    ...
    private fun showDirectory(guardian: Boolean, initialClass: String = "") {
        ...
        page.addView(tabs, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(46)))
        ...
        val toggle = TextView(this).apply {
            ...
            contentDescription = if (cardView) "Liste görünümü" else "Kart görünümü"
            textSize = 20f
            gravity = Gravity.CENTER
            setTextColor(blue)
            background = rounded(Color.WHITE, dp(14).toFloat(), soft)
        }
        searchRow.addView(toggle, marginLp(dp(46), dp(46), dp(7), 0, 0, 0))
        page.addView(searchRow)

        val classes = listOf("Tüm Sınıflar") + cache.listClasses().map { it.name }.filter { it.isNotBlank() }.distinct()
        var selectedIndex = classes.indexOf(selectedClass).takeIf { it >= 0 } ?: 0
        if (selectedIndex == 0) selectedClass = ""

        val filterRow = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
        }
        val selectorBox = FrameLayout(this).apply {
            background = rounded(Color.rgb(239, 244, 250), dp(13).toFloat())
        }
        val classText = TextView(this).apply {
            text = classes[selectedIndex]
            textSize = 13f
            gravity = Gravity.CENTER_VERTICAL
            setTextColor(ink)
            setPadding(dp(13), 0, dp(36), 0)
        }
        ...
        selectorBox.addView(classText, FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, dp(44)))
        selectorBox.addView(arrow, FrameLayout.LayoutParams(dp(36), dp(44), Gravity.END or Gravity.CENTER_VERTICAL))
        filterRow.addView(selectorBox, LinearLayout.LayoutParams(0, dp(44), 1f))
        ...
        filterRow.addView(groupButton, marginLp(dp(68), dp(44), dp(7), 0, 0, 0))
        ...
        fun updateGroupButton() {
            ...
            val item = cache.listClasses().firstOrNull { it.name == cls }
            val enabled = cls.isNotBlank() && !item?.whatsappUrl.isNullOrBlank()
            groupButton.isEnabled = enabled
            groupButton.alpha = if (enabled) 1f else .58f
            groupButton.background = rounded(if (enabled) green else passive, dp(13).toFloat())
        }

        fun render() {
            list.removeAllViews()
            val query = search.text?.toString().orEmpty().trim()
            val classFilter = if (query.isNotBlank()) "" else selectedClass
            if (guardian) {
                val rows = cache.listGuardians(query, classFilter)
                countText.text = rows.size.toString() + " veli"
        ...
        selectorBox.setOnClickListener {
            AlertDialog.Builder(this)
                .setTitle("Sınıf Seç")
                .setSingleChoiceItems(classes.toTypedArray(), selectedIndex) { dialog, which ->
                    selectedIndex = which
                    selectedClass = if (which == 0) "" else classes[which]
                    classText.text = classes[which]
                    render()
                    dialog.dismiss()
                }
        ...
        groupButton.setOnClickListener {
            val item = cache.listClasses().firstOrNull { it.name == selectedClass }
            if (!item?.whatsappUrl.isNullOrBlank()) openUri(item!!.whatsappUrl)
        }
        ...
        toggle.setOnClickListener {
            ...
            toggle.contentDescription = if (cardView) "Liste görünümü" else "Kart görünümü"
            Toast.makeText(this, if (cardView) "Kart görünümü" else "Liste görünümü", Toast.LENGTH_SHORT).show()
            render()
        }
        render()
        replaceContent(page)
    ...
    private fun guardianRow(g: CallerCache.Guardian): View {
        ...
        labels.addView(TextView(this).apply {
            text = listOf(g.studentName, g.className, if (g.schoolNo.isNotBlank()) "No: " + g.schoolNo else "").filter { it.isNotBlank() }.joinToString(" · ")
            textSize = 10.5f
            maxLines = 2
            setTextColor(muted)
        })
    ...
    private fun phoneRow(parent: LinearLayout, title: String, phone: String, color: Int) {
        ...
        val labels = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            addView(TextView(this@RehberActivity81).apply { text = title; textSize = 9.5f; setTextColor(muted) })
            addView(TextView(this@RehberActivity81).apply { text = PhoneUtil.display(phone); textSize = 13.5f; setTypeface(typeface, Typeface.BOLD); setTextColor(ink) })
        }
        row.addView(labels, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f))
    ...
    private fun guardianDetailCard(g: CallerCache.Guardian): View {
        ...
        return LinearLayout(this).apply {
            ...
            val labels = LinearLayout(this@RehberActivity81).apply {
                orientation = LinearLayout.VERTICAL
                addView(TextView(this@RehberActivity81).apply {
                ...
                if (g.phone.isNotBlank()) addView(TextView(this@RehberActivity81).apply {
        ...
        }.also { it.layoutParams = marginLp(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0, dp(4), 0, dp(4)) }
    }
    ...
    private fun showClasses() {
        ...
        classes.forEach { cls ->
            ...
            card.addView(TextView(this).apply { text = "›"; textSize = 26f; gravity = Gravity.CENTER; setTextColor(Color.LTGRAY) }, LinearLayout.LayoutParams(dp(32), dp(42)))
    ...
    private fun showAnnouncements() {
        ...
        rows.forEach { a ->
            val card = LinearLayout(this).apply {
                ...
                if (a.summary.isNotBlank()) addView(TextView(this@RehberActivity81).apply {
                    text = a.summary
                    textSize = 11.8f
                    setTextColor(muted)
                    setPadding(0, dp(5), 0, 0)
                })
                if (a.publishedAt.isNotBlank()) addView(TextView(this@RehberActivity81).apply {
                    text = a.publishedAt
                    textSize = 9.8f
                    setTextColor(orange)
    ...
    private fun showCalls() {
        ...
        val scroll = ScrollView(this)
        val body = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(0, 0, 0, dp(16)) }
        ...
        page.addView(plainButton("Arama Geçmişini Temizle", Color.rgb(232, 236, 242)) {
            AlertDialog.Builder(this).setTitle("Arama Geçmişi").setMessage("Uygulamadaki arama kayıtları temizlensin mi?")
                .setNegativeButton("Vazgeç", null)
                .setPositiveButton("Temizle") { _, _ -> cache.clearHistory(); showCalls() }.show()
    ...
    private fun showSettings() {
        ...
        settingsCard(body, "Arayan Kimliği", if (callerActive) "Açık · Gelen numara okul rehberinde yerel olarak eşleştirilir." else "Kapalı · Etkinleştirmek için dokunun.", if (callerActive) green else red) { requestCallScreeningRole() }
        val last = if (session.lastSync > 0) DateFormat.getDateTimeInstance(DateFormat.SHORT, DateFormat.SHORT).format(Date(session.lastSync)) else "Henüz yok"
        ...
        settingsCard(body, "Liste / Kart Görünümü", if (cardView) "Kart görünümü etkin" else "Liste görünümü etkin", turquoise) {
            cardView = !cardView; session.cardView = cardView
            Toast.makeText(this, if (cardView) "Kart görünümü" else "Liste görünümü", Toast.LENGTH_SHORT).show()
            showSettings()
        }
        settingsCard(body, "Güncelleme Kontrolü", "Akıllı Rehber sunucusundaki güncel sürümü kontrol edin.", orange) { checkAppVersion() }
        ...
        settingsCard(body, "Gizlilik ve Güvenlik", "Rehber oturumu Android Keystore ile şifrelenir; Play Protect-safe izin yapısı korunur.", Color.rgb(92, 108, 128), null)
        settingsCard(body, "ELAK Okulum · Akıllı Rehber", "v0.8.1 · Tasarım ve kullanım iyileştirmeleri", Color.rgb(92, 108, 128), null)
        scroll.addView(body)
        replaceContent(scroll)
    }
    ...
    private fun settingsCard(parent: LinearLayout, title: String, description: String, accent: Int, action: (() -> Unit)?) {
        ...
        card.addView(TextView(this).apply {
            text = "●"; textSize = 14f; gravity = Gravity.CENTER; setTextColor(accent)
        }, LinearLayout.LayoutParams(dp(28), dp(40)))
        val labels = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            addView(TextView(this@RehberActivity81).apply { text = title; textSize = 14.2f; setTypeface(typeface, Typeface.BOLD); setTextColor(ink) })
            addView(TextView(this@RehberActivity81).apply { text = description; textSize = 10.3f; setTextColor(muted); setPadding(0, dp(3), 0, 0) })
        }
        card.addView(labels, LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.WRAP_CONTENT, 1f))
        if (action != null) card.addView(TextView(this).apply { text = "›"; textSize = 24f; gravity = Gravity.CENTER; setTextColor(Color.LTGRAY) }, LinearLayout.LayoutParams(dp(28), dp(40)))
        parent.addView(card, marginLp(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT, 0, dp(4), 0, dp(4)))
    }

    private fun showSetup() {
        val box = LinearLayout(this).apply { orientation = LinearLayout.VERTICAL; setPadding(dp(24), dp(7), dp(24), 0) }
        box.addView(TextView(this).apply {
            text = "Okulum hesabıyla otomatik eşleşmediyse Akıllı Rehber hesabını yalnız bir kez tanımlayın. Şifre saklanmaz."
            textSize = 11.5f; setTextColor(muted)
        })
        ...
        val dialog = AlertDialog.Builder(this).setTitle("Rehber Güvenli Giriş").setView(box)
            ...
            .setPositiveButton("Giriş Yap ve Eşitle", null).create()
        dialog.setOnShowListener {
            dialog.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener {
                val username = user.text.toString().trim(); val password = pass.text.toString()
                if (username.isBlank() || password.isBlank()) { Toast.makeText(this, "Kullanıcı adı ve şifre gerekli.", Toast.LENGTH_SHORT).show(); return@setOnClickListener }
                dialog.getButton(AlertDialog.BUTTON_POSITIVE).isEnabled = false
                thread {
                    try {
                        val login = RehberApi.login(username, password)
                        session.token = login.token; session.username = username
    ...
    private fun checkAppVersion() {
        Toast.makeText(this, "Sürüm kontrol ediliyor…", Toast.LENGTH_SHORT).show()
        thread {
            try {
                val json = RehberApi.appVersion()
                val data = json.optJSONObject("data") ?: json
                val latest = listOf("version_name", "version", "latest_version").firstNotNullOfOrNull { key -> data.optString(key).takeIf { it.isNotBlank() } }.orEmpty()
                val message = if (latest.isBlank()) "Sunucuda sürüm bilgisi bulunamadı." else "Yüklü sürüm: 0.8.1\nSunucudaki sürüm: " + latest
                runOnUiThread { AlertDialog.Builder(this).setTitle("Güncelleme Kontrolü").setMessage(message).setPositiveButton("Tamam", null).show() }
            } catch (e: Exception) {
                runOnUiThread { Toast.makeText(this, "Sürüm kontrolü: " + (e.message ?: "başarısız"), Toast.LENGTH_LONG).show() }
            }
        }
    }
    ...
    private fun launchIncomingTest() {
        ...
        if (guardian == null) { Toast.makeText(this, "Test için önce veli kayıtlarını eşitleyin.", Toast.LENGTH_LONG).show(); return }
        val student = cache.getStudent(guardian.studentId)
        startActivity(Intent(this, IncomingCallerActivity::class.java).apply {
            putExtra("guardian", guardian.name); putExtra("relationship", guardian.relationship); putExtra("student", guardian.studentName)
            putExtra("school_no", guardian.schoolNo); putExtra("class_name", guardian.className); putExtra("student_id", guardian.studentId)
            putExtra("has_photo", student?.hasPhoto ?: false); putExtra("photo_version", student?.photoVersion ?: 0L); putExtra("phone", guardian.phone)
            putExtra("extra", 0); putExtra("related_students", listOf(guardian.studentName, guardian.className).filter { it.isNotBlank() }.joinToString(" · "))
        })
    }

    private fun openClassWhatsappGroups() {
        val rows = cache.listClasses().filter { it.whatsappUrl.isNotBlank() }
        if (rows.isEmpty()) { Toast.makeText(this, "Yetkili olduğunuz sınıflar için WhatsApp grup bağlantısı yok.", Toast.LENGTH_LONG).show(); return }
        AlertDialog.Builder(this).setTitle("Sınıf WhatsApp Grupları").setItems(rows.map { it.name }.toTypedArray()) { _, which -> openUri(rows[which].whatsappUrl) }.show()
    }
    ...
    private fun dial(phone: String) {
        val n = PhoneUtil.international(phone); if (n.isBlank()) return
    ...
    private fun openUri(url: String) {
        try { startActivity(Intent(Intent.ACTION_VIEW, Uri.parse(url))) } catch (_: Exception) { Toast.makeText(this, "Bağlantı açılamadı.", Toast.LENGTH_SHORT).show() }
    }

    private fun actionButton(iconRes: Int, color: Int, action: () -> Unit): View {
        return FrameLayout(this).apply {
            background = rounded(color, dp(11).toFloat()); setOnClickListener { action() }
            addView(ImageView(this@RehberActivity81).apply { setImageResource(iconRes); imageTintList = ColorStateList.valueOf(Color.WHITE); setPadding(dp(10), dp(10), dp(10), dp(10)) }, FrameLayout.LayoutParams(dp(40), dp(40), Gravity.CENTER))
            layoutParams = LinearLayout.LayoutParams(dp(41), dp(41)).apply { marginStart = dp(5) }
        }
    }

    private fun textActionButton(text: String, color: Int, action: () -> Unit): TextView {
        return TextView(this).apply {
            this.text = text; textSize = 13.5f; gravity = Gravity.CENTER; setTypeface(typeface, Typeface.BOLD); setTextColor(Color.WHITE); background = rounded(color, dp(11).toFloat()); setOnClickListener { action() }
            layoutParams = LinearLayout.LayoutParams(dp(41), dp(41)).apply { marginStart = dp(5) }
        }
    }
    ...
    private fun plainButton(text: String, color: Int, action: () -> Unit): TextView {
        ...
        return TextView(this).apply {
            this.text = text; textSize = 12.5f; gravity = Gravity.CENTER; setTypeface(typeface, Typeface.BOLD); setTextColor(if (pale) ink else Color.WHITE); background = rounded(color, dp(13).toFloat()); setOnClickListener { action() }
        }
    }

    private fun segment(text: String, selected: Boolean, action: () -> Unit): TextView {
        return TextView(this).apply {
            this.text = text; textSize = 11f; gravity = Gravity.CENTER; setTypeface(typeface, Typeface.BOLD); setTextColor(if (selected) Color.WHITE else ink); background = rounded(if (selected) red else Color.WHITE, dp(13).toFloat(), if (selected) red else soft); setOnClickListener { action() }
        }
    }

    private fun emptyState(parent: LinearLayout, text: String) {
        parent.addView(TextView(this).apply { this.text = text; textSize = 12.5f; gravity = Gravity.CENTER; setTextColor(muted); setPadding(dp(14), dp(34), dp(14), dp(34)) }, LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.WRAP_CONTENT))
    }

    private fun relationShort(value: String): String {
        val r = value.lowercase()
        return when { r.contains("anne") || r.contains("mother") -> "ANNE"; r.contains("baba") || r.contains("father") -> "BABA"; r.contains("öğr") || r.contains("ogr") || r.contains("student") -> "ÖĞR."; else -> "VELİ" }
    }

    private fun relationshipColor(value: String): Int {
        val r = value.lowercase()
        return when { r.contains("anne") || r.contains("mother") -> red; r.contains("baba") || r.contains("father") -> blue; else -> orange }
    }

    private fun replaceContent(view: View) {
        content.removeAllViews(); content.addView(view, FrameLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT))
    }

    private fun rounded(color: Int, radius: Float, strokeColor: Int? = null): GradientDrawable {
        return GradientDrawable().apply { setColor(color); cornerRadius = radius; if (strokeColor != null) setStroke(dp(1), strokeColor) }
    }

    private fun gradientTile(color: Int): GradientDrawable {
        return GradientDrawable(GradientDrawable.Orientation.TL_BR, intArrayOf(lighten(color, 1.06f), darken(color, .87f))).apply { cornerRadius = dp(22).toFloat() }
    }

    private fun darken(color: Int, factor: Float): Int = Color.rgb((Color.red(color) * factor).toInt().coerceIn(0,255), (Color.green(color) * factor).toInt().coerceIn(0,255), (Color.blue(color) * factor).toInt().coerceIn(0,255))
    private fun lighten(color: Int, factor: Float): Int = Color.rgb((Color.red(color) * factor).toInt().coerceIn(0,255), (Color.green(color) * factor).toInt().coerceIn(0,255), (Color.blue(color) * factor).toInt().coerceIn(0,255))
    private fun dp(value: Int): Int = (value * resources.displayMetrics.density + .5f).toInt()
    private fun marginLp(w: Int, h: Int, l: Int, t: Int, r: Int, b: Int): LinearLayout.LayoutParams = LinearLayout.LayoutParams(w, h).apply { setMargins(l,t,r,b) }
    private fun weightLp(weight: Float, left: Int, right: Int): LinearLayout.LayoutParams = LinearLayout.LayoutParams(0, ViewGroup.LayoutParams.MATCH_PARENT, weight).apply { setMargins(left,0,right,0) }

    @Deprecated("Deprecated in Java")
    override fun onBackPressed() { if (activeScreen != "home") showHome() else super.onBackPressed() }
